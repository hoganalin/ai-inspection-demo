...
const STATUS_CFG = {
  pass:    { label: '合格',   color: 'var(--matcha)',     mark: '○' },
  fail:    { label: '需檢修', color: 'var(--terracotta)', mark: '✕' },
  warning: { label: '留意',   color: 'var(--mustard)',    mark: '△' },
};

const SEV_COLOR: Record<string, string> = {
  low: 'var(--mustard)', medium: 'var(--clay)', high: 'var(--terracotta)',
};

const SEV_LABEL: Record<string, string> = { low: '輕微', medium: '中等', high: '嚴重' };

const FOUND_IN_CFG = {
  A:    { label: '僅 A',  bg: 'var(--clay-bg)',   color: 'var(--clay-deep)' },
  B:    { label: '僅 B',  bg: 'var(--indigo-bg)', color: 'var(--indigo)' },
  both: { label: '兩者',  bg: 'var(--mustard-bg)', color: 'var(--ink-soft)' },
};
...
const DropZone: React.FC<{
  ...
  return (
    <div
      ...
      style={{
        flex: 1, minWidth: 0,
        border: `1px ${preview ? 'solid' : 'dashed'} ${preview ? 'var(--line)' : 'var(--line-strong)'}`,
      ...
      {preview ? (
        <>
          ...
          <div style={{
            ...
            background: 'var(--paper-soft)', color,
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
          ...
          <span style={{ fontFamily: 'var(--font-serif)', fontSize: 13, color: 'var(--ink)', letterSpacing: '0.1em' }}>
            {label === 'A' ? '基準樣本' : '對照樣本'}
          </span>
          <span className="label-en" style={{ fontSize: 9 }}>點擊或拖放</span>
        </div>
      )}
...
const MiniResult: React.FC<{ label: string; result: InspectionResult; color: string }> = ({ label, result, color }) => {
  ...
  return (
    <div style={{ flex: 1, padding: '14px 18px', background: 'var(--paper-soft)', border: '1px solid var(--line)', borderRadius: 'var(--r-md)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
        ...
        <span style={{
          marginLeft: 'auto', fontFamily: 'var(--font-serif)', fontSize: 12,
          color: cfg.color, letterSpacing: '0.1em',
        }}>
          <span style={{ marginRight: 4 }}>{cfg.mark}</span>{cfg.label}
        </span>
      </div>
      <div style={{ display: 'flex', gap: 20, marginBottom: 10 }}>
        <div>
          ...
          <div className="num-mono" style={{ fontSize: 20, color: cfg.color, fontWeight: 500, marginTop: 2 }}>
            {result.confidence}<span style={{ fontSize: 11, color: 'var(--ink-mute)', marginLeft: 2 }}>%</span>
          </div>
        </div>
        <div>
          <div className="label-en" style={{ fontSize: 9 }}>瑕疵數</div>
          <div className="num-mono" style={{ fontSize: 20, color: result.defects.length > 0 ? 'var(--terracotta)' : 'var(--matcha)', fontWeight: 500, marginTop: 2 }}>
            {String(result.defects.length).padStart(2, '0')}
          </div>
        </div>
      </div>
      <p style={{ fontSize: 12, color: 'var(--ink-soft)', lineHeight: 1.7 }}>{result.summary}</p>
    </div>
  );
};

/* ── Difference Row ── */
const DiffRow: React.FC<{ diff: ComparisonDefect; index: number; isLast: boolean }> = ({ diff, index, isLast }) => {
  const fi = FOUND_IN_CFG[diff.foundIn];
  return (
    <div style={{
      display: 'grid', gridTemplateColumns: '40px 60px 1fr 70px',
      gap: 14, padding: '14px 18px',
      borderBottom: isLast ? 'none' : '1px solid var(--line-soft)',
      alignItems: 'start',
    }}>
      <div className="num-mono" style={{ fontSize: 11, color: 'var(--ink-mute)', paddingTop: 2 }}>
        {String(index + 1).padStart(2, '0')}
      </div>
      <div>
        <span style={{
          fontFamily: 'var(--font-serif)', fontSize: 12,
          color: fi.color, letterSpacing: '0.1em',
        }}>{fi.label}</span>
      </div>
      <div>
        <div style={{ fontFamily: 'var(--font-serif)', fontSize: 13, fontWeight: 500, color: 'var(--ink)', marginBottom: 3 }}>
          {diff.location}
        </div>
        <div style={{ fontSize: 12, color: 'var(--ink-soft)', lineHeight: 1.7 }}>{diff.description}</div>
      </div>
      <div style={{ textAlign: 'right' }}>
        <span style={{
          fontFamily: 'var(--font-serif)',
          fontSize: 11,
          color: SEV_COLOR[diff.severity],
          padding: '2px 8px',
          border: `1px solid ${SEV_COLOR[diff.severity]}66`,
          borderRadius: 2,
          letterSpacing: '0.1em',
          background: 'var(--paper-soft)',
        }}>
          {SEV_LABEL[diff.severity] ?? diff.severity}
        </span>
      </div>
    </div>
  );
...
export const ComparisonPanel: React.FC<Props> = ({ customCriteria }) => {
  ...
  const simColor = result
    ? (result.similarity >= 70 ? 'var(--matcha)' : result.similarity >= 40 ? 'var(--mustard)' : 'var(--terracotta)')
    : 'var(--clay)';

  return (
    <div className="anim-in" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div className="divider">對照分析 · COMPARE</div>
      ...
      <button
        ...
        style={{ fontSize: 12, alignSelf: 'flex-start', opacity: loadingDemo ? 0.5 : 1 }}
      >
        {loadingDemo
          ? <><span className="soft-loader" style={{ marginRight: 8 }}><span/><span/><span/></span>載入中…</>
          : <>載入 PCB 示範圖片（正常 vs 鏽蝕）</>
        }
      </button>

      {/* Upload Row */}
      <div style={{ display: 'flex', gap: 12 }}>
        <DropZone label="A" preview={previewA} color="var(--clay)"
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, flexShrink: 0, minWidth: 24 }}>
          <div style={{ width: 1, flex: 1, background: 'var(--line)' }} />
          <span className="label-en" style={{ fontSize: 10 }}>VS</span>
          <div style={{ width: 1, flex: 1, background: 'var(--line)' }} />
        </div>
        <DropZone label="B" preview={previewB} color="var(--indigo)"
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button
          ...
          style={{ flex: 1, padding: '12px', opacity: canCompare ? 1 : 0.4 }}
        >
          {status === 'analyzing' ? (
      ...
      {result && (
        <div className="anim-in" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div className="divider">相似度 · SIMILARITY</div>
          <div style={{ padding: '20px 24px', background: 'var(--paper-soft)', border: '1px solid var(--line)', borderRadius: 'var(--r-md)', display: 'flex', alignItems: 'center', gap: 20 }}>
            <div style={{ flex: 1 }}>
              <div className="progress" style={{ height: 3 }}>
                <div className="progress-fill" style={{ width: `${result.similarity}%`, background: simColor }} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
                <div className="label-en" style={{ fontSize: 9 }}>差異</div>
                <div className="label-en" style={{ fontSize: 9 }}>一致</div>
              </div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div className="num-mono" style={{ fontSize: 32, color: simColor, lineHeight: 1 }}>
                {result.similarity}<span style={{ fontSize: 14, color: 'var(--ink-mute)' }}>%</span>
              </div>
              <div className="label-en" style={{ fontSize: 9, marginTop: 4 }}>
                {result.similarity >= 70 ? '類似' : result.similarity >= 40 ? '部分相近' : '差異明顯'}
              </div>
            </div>
          </div>

          <div style={{ display: 'flex', gap: 12 }}>
            <MiniResult label="A" result={result.imageA} color="var(--clay)" />
            <MiniResult label="B" result={result.imageB} color="var(--indigo)" />
          </div>

          {result.differences.length > 0 && (
            <>
              <div className="divider">差異點 · DIFFERENCES</div>
              <div style={{ background: 'var(--paper-soft)', border: '1px solid var(--line)', borderRadius: 'var(--r-md)', padding: 0 }}>
                <div style={{ padding: '12px 18px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div className="label-en">差異明細 · DIFF LIST</div>
                  <div className="num-mono" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>
                    {String(result.differences.length).padStart(2, '0')} 件
                  </div>
                </div>
                {result.differences.map((d, i) => (
                  <DiffRow key={i} diff={d} index={i} isLast={i === result.differences.length - 1} />
                ))}
              </div>
            </>
          )}
          ...
          {result.differences.length === 0 && (
            <div style={{ padding: '24px', textAlign: 'center', background: 'var(--matcha-bg)', border: '1px solid var(--matcha)', borderRadius: 'var(--r-md)' }}>
              <div style={{ fontFamily: 'var(--font-serif)', fontSize: 20, color: 'var(--matcha)', marginBottom: 6 }}>○</div>
              <p style={{ fontSize: 13, color: 'var(--ink)', fontFamily: 'var(--font-serif)', letterSpacing: '0.1em' }}>兩張圖片無明顯差異</p>
            </div>
          )}
      ...
      {status === 'error' && (
        <div style={{ padding: '20px', textAlign: 'center', background: 'var(--terracotta-bg)', border: '1px solid var(--terracotta)', borderRadius: 'var(--r-md)' }}>
          <p style={{ fontSize: 13, color: 'var(--terracotta)' }}>比對失敗，請稍後再試或檢查伺服器設定。</p>
        </div>
      )}
    </div>
  );
